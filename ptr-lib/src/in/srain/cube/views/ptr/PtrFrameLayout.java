...
public class PtrFrameLayout extends ViewGroup {
    ...
    // config
    private int mDurationToOffset = 200;  //偏移到offset的时间
    private int mDurationToClose = 1000; //关闭头部的时间
    ...
    private boolean mLoadFinishComplete = false;//加载结束<限制加载条数>
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        if (arr != null) {
            ...
            mDurationToOffset = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_offset, mDurationToOffset);
    ...
    protected void onAttachedToWindow() {
        ...
        if (mHeaderView == null)
            onFinishInflate();
        //点击加载事件
        if (mClickToLoad && !mLoadFinishComplete) {
            mFooterView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    mFooterView.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            performClickLoad();
                            mScrollChecker.tryToScrollTo(0, mDurationToOffset, true);
                        }
                    }, 1000);
                }
            });
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                //处理连续点击Bug
                if (mStatus == PTR_STATUS_COMPLETE && (mPtrIndicator.hasLeftStartPosition() || mPtrIndicator.hasLeftBottomPosition())) {
                    return true;
                }
    ...
    private boolean isIntercepterView(boolean isHead) {
        boolean isIntecepter=false;
        if((mStatus==PTR_STATUS_LOADING||mStatus==PTR_STATUS_COMPLETE)&&!mPtrIndicator.isInStartPosition()) {
            if(mPtrIndicator.hasLeftStartPosition())
                isIntecepter = true&&isHead;
            else
                isIntecepter=true&&!isHead;
        }else if(mStatus==PTR_STATUS_INIT||mStatus==PTR_STATUS_PREPARE){
...
//            }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
                (mPtrIndicator.goDownCrossFinishPosition() && mStatus == PTR_STATUS_COMPLETE && isEnabledNextPtrAtOnce() && isUnderTouch)) {
            mStatus = PTR_STATUS_PREPARE;
        ...
        if (mStatus == PTR_STATUS_LOADING) {
            ...
            if (mKeepHeaderWhenRefresh) {
                ...
                if (mPtrIndicator.isOverOffsetToKeepHeaderWhileLoading() && !stayForLoading) {
                    mScrollChecker.tryToScrollTo(mPtrIndicator.getOffsetToKeepHeaderWhileLoading(), mDurationToOffset, false);
                } else {
                    // do nothing
                }
            } else {
        ...
        if (mStatus == PTR_STATUS_LOADING) {
            ...
            if (mKeepFooterWhenRefresh) {// TODO: 2017/8/25
                ...
                if (mPtrIndicator.isOverOffsetToKeepFooterWhileLoading() && !stayForLoading) {
                    //表示移动到啥位置《底层封装好的》
                    mScrollChecker.tryToScrollTo(-mPtrIndicator.getOffsetToKeepFooterWhileLoading(), mDurationToOffset, true);
                }
            } else {
            ...
            if (mStatus == PTR_STATUS_COMPLETE) {
                ...
                if (mPtrIndicator.getCurrentPosY() == -mFooterHeight && mClickToLoad && !mLoadFinishComplete) {
                    PtrIndicator.isPullHeader = false;
    ...
    private void tryScrollBackToTop() {
        mScrollChecker.tryToScrollTo(PtrIndicator.POS_START, mDurationToClose, false);
...
//        }
    ...
    private void tryScrollBackToBottom() {
        mScrollChecker.tryToScrollTo(PtrIndicator.POS_START, mDurationToClose, true);
//        if (!mPtrIndicator.isUnderTouch()) {
//            mScrollChecker.tryToScrollTo(PtrIndicator.POS_START, mDurationToClose,true);
//        }
    }
    ...
    public void autoRefresh() {
        autoRefresh(true, mDurationToClose);
    }

    public void autoRefresh(boolean atOnce) {
        autoRefresh(atOnce, mDurationToClose);
    }
    ...
    public void autoRefresh(boolean atOnce, int duration) {
        ...
        PtrIndicator.isPullHeader = true;
        PtrIndicator.isPullFooter = false;
        mStatus = PTR_STATUS_PREPARE;
        if (mPtrUIHeaderHolder.hasHandler()) {
            mPtrUIHeaderHolder.onUIRefreshPrepare(this);
     ...
     * 向外提供默认的统一接口实现上拉下拉刷新
     */
    public void setPtrDefaultRefreshLoadHandler(PtrDefaultRefreshLoadHandler ptrDefaultRefreshLoadHandler) {
        mPtrRefreshHandler = ptrDefaultRefreshLoadHandler;
        mPtrLoadHandler = ptrDefaultRefreshLoadHandler;
    }
    ...
    public void setResistance(float resistance) {
        if(resistance<=1.5f)resistance=1.5f;
        mPtrIndicator.setResistance(resistance);
    }

    @SuppressWarnings({"unused"})
    public float getDurationToOffset() {
        return mDurationToOffset;
    }
     ...
     * @param duration
     */
    public void setDurationToOffset(int duration) {
        if(duration<200) duration=200;
        mDurationToOffset = duration;
    }

    @SuppressWarnings({"unused"})
    public long getDurationToClose() {
    ...
    public boolean isLoadFinishComplete() {
        return mLoadFinishComplete;
    }

    public void setLoadFinishComplete(boolean loadFinishComplete) {
        this.mLoadFinishComplete = loadFinishComplete;
    }

    private boolean isCanRefreshOrLoad(boolean isPullToLoad){
        if(isLoadFinishComplete()&&isPullToLoad)  return true;
        if (mStatus == PTR_STATUS_LOADING || (mStatus != PTR_STATUS_COMPLETE && isAutoRefresh()))
           return false;
        return true;
    }
    ...
    class ScrollChecker implements Runnable {
        ...
        private void reset() {
            ...
            if(!isCanRefreshOrLoad(isPullToLoad)) return;
            PtrIndicator.isPullFooter = false;
            PtrIndicator.isPullHeader = false;
            isPullToLoad = false;
        }
